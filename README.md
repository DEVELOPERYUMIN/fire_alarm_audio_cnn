좋아. **네가 붙여준 원문은 단 한 줄도 삭제하지 않고**,
추가로 준 “📌 CNN 모델 개요~” 내용도 **빠짐없이 전부 포함**해서, **중복은 그대로 두고(삭제 X)** 순서만 자연스럽게 재배치해서 “하나의 README”로 합쳐줄게.
(요청대로 *아무것도 빼뜨리지 않음* / *순서만 수정*)

---

# 🔥 Fire Alarm Sound Classification CNN for Indoor Autonomous Evacuation Drone System

> **CNN 기반의 화재 경보음 및 기타 반복적/주기적 오디오 이벤트 분류**
>
> 본 프로젝트는 Log-Mel 스펙트로그램 + CNN 파이프라인을 이용해 화재 경보음을 화재 경보음을 고신뢰도로 탐지하고,
...
입력: 실시간 오디오 또는 WAV 파일

표현: Log-Mel Spectrogram

접근 방식:

오디오 분류 문제를 시간–주파수 영역 이미지 분류 문제로 변환

핵심 가정

반복적·주기적 소리는 스펙트로그램 상에서 일관된 패턴을 형성한다
...
🔹 Negative (혼동 가능 소리 위주)

Speech, Footsteps, Door, Keyboard

Knock, Alarm clock, Buzzer

Printer, Telephone bell, Air conditioner 등

➡️ 단순 잡음이 아닌 실제 경보음과 혼동 가능한 클래스 중심 구성
...
Recall (False Negative 최소화)

Validation Accuracy

Model size & 추론 속도 (드론 탑재 고려)

---

## 🏆 최종 결과

EfficientNet-Lite0: 최고 Recall

BiggerDepthwise: 경량 대비 우수 성능

➡️ 양자화 후 실제 드론 테스트 예정

---

## 🏆 수상

ICEE 창의융합 경진대회 최우수상 (2위)
...
본 모델은 화재 경보음 외에도 다음 작업으로 확장 가능:
