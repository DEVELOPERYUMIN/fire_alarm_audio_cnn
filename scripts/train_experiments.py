#!/usr/bin/env python3
# scripts/train_experiments.py ── MobileNetV2 (0.5×) + EarlyStopping + Epoch CSV + plots
# ------------------------------------------------------------------
...
# ➀ MobileNetV2 (0.5×) ---------------------------------------------------
def get_mobilenet(drop=0.3):
    return timm.create_model(
        "mobilenetv2_050", pretrained=True,
...
def train_and_eval(args):
    ...
    model=get_mobilenet(args.dropout).to(dev)
...
def get_parser():
    ...
    ap.add_argument('--model_name',default='mobilenetv2_050'); ap.add_argument('--exp_idx',type=int,default=1)
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>1:
        ...
        dropouts=[0.3,0.5]; lrs=[1e-3, 5e-4]; pos_w=[1.3,1.4, 1.55]
        grid=itertools.product(dropouts,lrs,pos_w,[42])
        csv_path=os.path.join(RESULTS_DIR,"mobilenetv2_grid.csv")
        total=len(dropouts)*len(lrs)*len(pos_w); print("Total exp:",total)
        exp=1
        for d,lr,pw,_ in grid:
            tag=f"mobilenetv2_050_exp{exp:03d}.pt"
